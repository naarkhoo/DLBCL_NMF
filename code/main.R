...
{
maindir = "/home/superiois/Downloads/projectx3/"
...
data1 = read.csv(address("DLBCL_NMF/data/DLBCL_OE_L1000_NMF_TA.OE002_A375_72H.csv"), sep = ",")
data1 = t(data1)
data1 = as.data.frame(data1)
colnames(data1) = as.matrix(data1[1,])
data1 = data1[-1,]
CNMF.write.gct.2(data1, descs = ",", filename = address("DLBCL_NMF/data/DLBCL_OE_L1000_NMF_TA.OE002_A375_72H.gct"))


source(address("DLBCL_NMF/code/DISSECTOR_lib.v3.R"))
DISSECTOR_explore_component_creation.v1(
	input_dataset             = address("DLBCL_NMF/data/DLBCL_OE_L1000_NMF_TA.OE002_A375_72H.gct"), # Input A dataset (GCT)
	...
	output_plots              = address("DLBCL_NMF/plots/DLBCL_OE_L1000_NMF_TA.OE002_A375_72H.EXPLORE.PLOTS.v1.pdf"))  # Output plots with results


source(address("DLBCL_NMF/code/DISSECTOR_lib.v3.R"))
DISSECTOR_create_components.v1(                 # Project an input dataset into components using NMF  (A ~ W x H)
	input_dataset          = address("DLBCL_NMF/data/DLBCL_OE_L1000_NMF_TA.OE002_A375_72H.gct"),
	input_normalization    = "rank",        # Normalization for the input dataset: "rank"
	...
	output_plots           = address("DLBCL_NMF/plots/compound_A375_7.PLOTS.pdf"),   # Output plots file (PDF, W, H and A)
	output_W_dataset       = address("DLBCL_NMF/outputs/compound_A375_7.W.gct"),     # Output W matrix (GCT file)
	output_H_dataset       = address("DLBCL_NMF/outputs/compound_A375_7.H.gct"),     # Output H matrix (GCT file)
	output_H_w_dataset     = address("DLBCL_NMF/outputs/compound_A375_7.H_w.gct"))   # Output W-derived H matrix (GCT file)


source(address("DLBCL_NMF/code/DISSECTOR_lib.v3.R"))
DISSECTOR_make_heatmap_of_matrix.v1(
	input_dataset            = address("DLBCL_NMF/outputs/compound_A375_7.H_w.gct"), # Input dataset (GCT).
	annot_file               = c(address("DLBCL_NMF/data/compound_classes_A375_7.v3.csv"), "Sample", "Type", T),
	...
	output_plot_landscape    = address("DLBCL_NMF/plots/A375_DLBCL_n978x323.H_w_LPLOT.pdf"),  # Output (PDF) file
	output_plot_portrait     = address("DLBCL_NMF/plots/A375_DLBCL_n978x323.H_w_PPLOT.pdf"))  # Output (PDF) file
...
DISSECTOR_project_dataset.v1(     # Project a dataset in the space defined by a W matrix
	...
	input_W_dataset         = address("DLBCL_NMF/outputs/compound_A375_7.W.gct"),  # my W Input W matrix (GCT)
	W_normalization         = "none",       # Normalization for W                                            
	output_H_dataset        = address("DLBCL_NMF/outputs/DLBCL_OE_L1000_NMF_TA.OE002_A375_72H_H.gct"))



source(address("DLBCL_NMF/code/DISSECTOR_lib.v3.R"))
DISSECTOR_make_heatmap_of_matrix.v1(
	input_dataset            = address("DLBCL_NMF/outputs/DLBCL_OE_L1000_NMF_TA.OE002_PC3_72H_H.gct"), # Input dataset (GCT).
	annot_file               = c(address("DLBCL_NMF/data/CCLE_Kim_Tamayo_96AffyCEL_FAZES_Oct12.v2.txt"),"CCLE_name", "Site.Primary", F),
	transpose_data           = F,           # Transpose input matrix
	append_annot             = T,           # Append annotation to column names
	sorting_method           = "HC",        # Sorting method for cols inside phenotype: MDS (Multi_dimensinal Scaling) or HC (Hiererachical Clustering)  
	output_plot_landscape    = address("DLBCL_NMF/plots/ccle_DLBCL_H_proj_LPLOT.v1.pdf"),  # Output (PDF) file
	output_plot_portrait     = address("DLBCL_NMF/plots/ccle_DLBCL_H_proj_PPLOT.v1.pdf"))  # Output (PDF) file


source(address("DLBCL_NMF/code/DISSECTOR_lib.v3.R"))
DISSECTOR_generate_association_matrix.v1(         # generate the association matrix between the columns (perturbation/states)
	input_dataset            = address("DLBCL_NMF/outputs/compound_A375_7.H_w.gct"), # Input dataset (GCT).
	input_dataset2           = address("DLBCL_NMF/outputs/compound_A375_7.H_w.gct"), # Input dataset (GCT).
	association_type         = "columns",     # Type of association: between "columns" or between "rows"                                            
	annot_file               = c(address("DLBCL_NMF/data/compound_classes_A375_7.v3.csv"), "Gene.Symbol", "Pathway.Cellular.Process", T),
	annot_file2              = c(address("DLBCL_NMF/data/compound_classes_A375_7.v3.csv"), "Gene.Symbol", "Pathway.Cellular.Process", T),  
	association_metric       = "IC",          # Association metric: "IC" (Information Coefficient), "COR" (Pearson), "SPEAR" (Spearman)
	output_assoc_matrix_file = address("DLBCL_NMF/outputs/DLBCL_OE_L1000_NMF_TA.OE002_A375_72H.H_w_ASSOC.v1.gct"), 
	output_assoc_plot        = address("DLBCL_NMF/outputs/DLBCL_OE_L1000_NMF_TA.OE002_A375_72H.H_w_ASSOC.v1.pdf"))  # Output (PDF) assoc plot

source(address("DLBCL_NMF/code/DISSECTOR_lib.v3.R"))
DISSECTOR_find_top_associations.v1(   #  Find top associations to each column state 
	input_dataset            = address("DLBCL_NMF/outputs/compound_A375_7.H_w.gct"),
	annot_file               = c(address("DLBCL_NMF/data/compound_classes_A375_7.v3.csv"), "Gene.Symbol", "Pathway.Cellular.Process", T),  
	n.top                    = 35,          # Number of top (UP) matches to display
	n.bottom                 = 10,          # Number of top (DOWN) matches to display
	output_top_assocs_plot   = address("DLBCL_NMF/plots/DLBCL_OE_L1000_NMF_TA.OE002_A375_72H.H_TOP_ASSOC.v1.offset.pdf")) # Output (PDF) file with top associations
...
DISSECTOR_produce_MDS_and_network.v1(    # Make MDS projection and network from a GCT file
	input_dataset          = address("DLBCL_NMF/outputs/compound_A375_7.H_w.gct"),
	col_annot_file         = c(address("DLBCL_NMF/data/compound_classes_A375_7.v3.csv"), "Gene.Symbol", "Pathway.Cellular.Process", T),  
	...
	output_plot            = address("DLBCL_NMF/plots/DLBCL_OE_L1000_NMF_TA.OE002_A375_72H.H_w.v1.MDS.pdf"), 
	MDS_movie_file         = NULL)
